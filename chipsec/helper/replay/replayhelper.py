...
class ReplayHelper(Helper):
    ...
    def _get_element(self, cmd: str, args: Tuple) -> Optional[Any]:
        ...
        logger().log_error("Missing entry for {} {}".format(str(cmd), targs))
        return None
    
    def _load(self) -> None:
        file_data = read_file(self.config_file)
        if file_data == 0:
            raise OsHelperError(f"Unable to open JSON File: {self.config_file}")
        try:
            self._data = loads(file_data)
        except:
            raise OsHelperError(f'Unable to load JSON File: {self.config_file}')
